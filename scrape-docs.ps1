# OpenClaw zh-CN 文档下载脚本
# 从 sitemap.xml 中提取所有 zh-CN 页面并下载为 Markdown 格式

$baseUrl = "https://docs.openclaw.ai"
$outputDir = "d:\other-work\openclaw-index\docs"

# 所有 zh-CN 页面路径（从 sitemap.xml 中提取）
$pages = @(
    "zh-CN"
    "zh-CN/AGENTS"
...
Write-Host "共 $($pages.Count) 个页面需要下载" -ForegroundColor Cyan
...
foreach ($page in $pages) {
    ...
    # 将 zh-CN/xxx/yyy 转为 docs/xxx/yyy.html 的路径结构
    $relativePath = $page -replace "^zh-CN/?", ""
    if ($relativePath -eq "") {
        $filePath = Join-Path $outputDir "index.html"
    } else {
        $filePath = Join-Path $outputDir "$relativePath.html"
    }
    ...
    try {
        ...
        if ($completed % 10 -eq 0) {
            Write-Host "  已完成 $completed / $($pages.Count)" -ForegroundColor Green
        }
    } catch {
        $failed += $page
        Write-Host "  失败: $page - $($_.Exception.Message)" -ForegroundColor Red
    }
...
$msg = "Download done! OK: $completed, Fail: $($failed.Count)"
